...
2. 应用层(application), 定义系统可以完成的工作。它并不处理业务逻辑通过协调领域对象或领域服务完成业务逻辑, 并通过输入端口输出结果。也可以在这一层进行事物管理。
...
### 实体 (Entities)
>不同于领域驱动设计里的实体概念，而偏向于业务的逻辑，比如贷款利率的计算规则
...
此层的软件包含 _应用专属_ 的业务规则, 它封装和实现的系统所有的用例, 这些用例协调进出实体的数据流, 并且指导这些实体利用其本身 _企业范围_ 的业务规则实现用例的目标。
...
![干净架构控制流](/clean_flow.png)
>开放箭头表示"关联/使用"的关系，闭合箭头(空三角)表示实现或继承的关系

1. 用户通过 view 与系统交互
2. View 发起请求到 Controller
3. Controller 将请求封装为 request 模型(由Use case定义)通过输入端口(Request Interface)传递到 use case
4. Use case 解析请求并生成 response 模型通过输出端口传递到 Presenter(演示器)
5. Presenter 解析 response 生成 view 模型并传递到 View
6. 用户通过 View 看到了响应的结果
